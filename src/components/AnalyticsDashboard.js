...
const AnalyticsDashboard = () => {
  ...
  useEffect(() => {
    ...
    const inProgressReports = savedReports.filter(r => r.status === 'inProgress').length;
            ...
            {reports.slice(0, 5).map((report, index) => (
              <div key={report.id} className="flex items-center justify-between p-4 glass-morphism rounded-2xl border border-white/10 animate-slide-up" style={{ animationDelay: `${index * 0.1}s` }}>
                ...
                <span className={`px-3 py-1.5 rounded-xl text-xs font-bold shadow-soft ${
                  report.status === 'open' ? 'bg-gradient-to-r from-red-400 to-red-500 text-white' :
                  report.status === 'inProgress' ? 'bg-gradient-to-r from-yellow-400 to-orange-400 text-white' :
